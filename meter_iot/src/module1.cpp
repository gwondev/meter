 ...
 *
 * ---------------------------------------------------------------------------
 * [배포 확인용 BUILD_TAG — 이후 IoT 수정 시에도 반드시 유지]
 *
 * 1) 이 파일을 고칠 때마다 BUILD_TAG 문자열을 바꿔라. (날짜·변경요약 포함 권장)
 * 2) setup() 에서 Serial 로 딱 한 번만 출력한다. (루프에서 반복 출력 금지)
 * 3) 업로드 검증 절차:
 *      pio run -t clean
 *      pio run -t upload
 *      pio device monitor -b 115200
 *    → 부팅 직후 BUILD_TAG=... 한 줄이 보이면 새 펌웨어가 올라간 것.
 *    → 안 보이거나 옛 태그가 보이면 업로드 실패 / 잘못된 보드 / 옛 바이너리.
 * 4) AI·개발자는 IoT 코드를 수정할 때마다 이 규칙을 반복한다.
 * ---------------------------------------------------------------------------
 */
#include <Arduino.h>
#include <cstdio>
...
/*
 * 펌웨어 수정 문자 — clean + upload 후 시리얼에 한 번 출력되어 업로드 성공을 증명한다.
 * 코드 변경 시 반드시 새 값으로 갱신할 것. (예: METER-IOT-m1-YYYYMMDD-요약)
 */
static const char *const BUILD_TAG = "METER-IOT-m1-20260901-LED-RYB";
...
static void updateStatusLed() {
  ...
  if (!s_mqtt_connected) {
    setLedRgb(true, false, false); /* 빨강 — MQTT 연결 실패 */
  } else if (now < s_ledFlashUntilMs) {
    setLedRgb(false, false, true); /* 파랑 — MQTT 전송 성공 */
  } else {
    setLedRgb(true, true, false); /* 노랑 — MQTT 연결 성공 유지 */
  }
}
...
void setup() {
  ...
  /* 배포 확인: clean+upload 후 이 한 줄이 보이면 새 펌웨어 */
  Serial.printf("BUILD_TAG=%s\n", BUILD_TAG);
  Serial.printf("MODULE_SERIAL=%s  PUB: meter/%s/status (%lus)\n",
                MODULE_SERIAL, MODULE_SERIAL, PUBLISH_INTERVAL_MS / 1000UL);
  Serial.printf("build %s %s\n", __DATE__, __TIME__);
  Serial.println("LED: RED=mqtt fail / YELLOW=mqtt ok / BLUE=pub ok");
  Serial.println("====================================");
  ...
  setLedRgb(true, false, false); /* 부팅 직후 빨강 */
